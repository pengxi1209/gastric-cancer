...
$suffix ||="PASS.hg38_multianno.txt";
...
while (<INL>){
        ...
        next if $.==1;  # 跳过表头
        my @fields = split /\t/;
        next unless @fields >= 2;  # 至少需要2列
        
        my $GlimsID = $fields[0];
        my $sample = $fields[1];  # Comparisons列，实际上就是样本名
        my $Tumor_Sample_Barcode = $fields[2] || $sample;  # 如果没有第3列，使用样本名
        
        # 去除空格
        $GlimsID =~ s/\s+//g;
        $sample =~ s/\s+//g;
        $Tumor_Sample_Barcode =~ s/\s+//g;
        
        next unless $sample;  # 跳过空样本名
        ...
        my $f="$indir/$sample/$sample\_$suffix";
        unless (-e $f) {
                print STDERR "Warning: File not found: $f\n";
                close Sub;
                next;
        }
        ...
        while (<IN>){
                ...
                # 这是tumor-only单样本数据，从Otherinfo13读取
                # 格式: GT:AD:AF:DP:F1R2:F2R1:FAD:SB
                # 示例: 0/1:43,4:0.111:47:11,1:12,1:23,2:22,21,2,2
                my $t_ref_count = 0;
                my $t_alt_count = 0;
                my $t_depth = 0;
                my $n_depth = 0;  # tumor-only数据没有normal
                
                if (exists $column{'Otherinfo13'} && defined $t[$column{'Otherinfo13'}]) {
                        my @tumor_fields = split /:/, $t[$column{'Otherinfo13'}];
                        # AD是第2个字段（index 1），格式为 ref,alt
                        if (@tumor_fields > 1 && $tumor_fields[1] =~ /,/) {
                                ($t_ref_count,$t_alt_count) = split /,/, $tumor_fields[1];
                                $t_depth = $t_ref_count + $t_alt_count;
                        }
                }
                
                my $TumorVAF = $t_depth > 0 ? $t_alt_count/$t_depth : 0;
